...
sDF.write.format('delta').option("mergeSchema", "true").partitionBy("Unit").mode('overwrite').saveAsTable("ademianczuk.hls.b_staff")

# COMMAND ----------

#Write the historical callout data for backfill
coDF = spark.createDataFrame(calloutDF)
coDF.write.format('delta').option("mergeSchema", "true").partitionBy("Facility").mode('overwrite').saveAsTable("ademianczuk.hls.b_callout")
